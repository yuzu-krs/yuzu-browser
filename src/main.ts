...
function setupFileMetaTool(): void {
  ...
  const tabs = Array.from(
    document.querySelectorAll<HTMLButtonElement>("[data-fm-tab]"),
  );
  const panels = Array.from(
    document.querySelectorAll<HTMLElement>("[data-fm-panel]"),
  );
  const badge = document.getElementById("fm-type-badge");
  const activateTab = (name: string): void => {
    tabs.forEach((t) => t.classList.toggle("active", t.dataset.fmTab === name));
    panels.forEach((p) => {
      p.hidden = p.dataset.fmPanel !== name;
    });
  };
  tabs.forEach((t) =>
    t.addEventListener("click", () => {
      const name = t.dataset.fmTab;
      if (name) activateTab(name);
    }),
  );
  const updateTypeHints = (p: string): void => {
    const ext =
      (p.split(/[\\/]/).pop() || "").split(".").pop()?.toLowerCase() ?? "";
    tabs.forEach((t) => t.classList.remove("suggested"));
    ...
    if (audioExts.has(ext)) {
      suggested = "audio";
      label = `音声 (${ext.toUpperCase()})`;
    } else if (ext === "png") {
      suggested = "image";
      label = "画像 (PNG)";
      const pt = tabs.find((t) => t.dataset.fmTab === "pngtext");
      pt?.classList.add("suggested");
    } else if (
      ["jpg", "jpeg", "webp", "tiff", "tif", "heic", "heif"].includes(ext)
    ) {
      suggested = "image";
      label = `画像 (${ext.toUpperCase()})`;
    } else if (
      [
        "pdf",
        "docx",
    ...
    if (suggested) {
      const sg = tabs.find((t) => t.dataset.fmTab === suggested);
      sg?.classList.add("suggested");
    }
    if (badge) {
      if (label) {
  ...
  const load = async (): Promise<void> => {
    ...
    updateTypeHints(path.value.trim());
    try {
...
function setupImgStdFavicon(): void {
  ...
  runBtn.addEventListener("click", () => {
    ...
    allBtn.addEventListener("click", () => {
      void buildIco(canvases).then((blob) => downloadBlob(blob, "favicon.ico"));
    });
...
void setupAIMultiTranslate;
void setupImgStdEditor;
void setupVideoStudioTool;
